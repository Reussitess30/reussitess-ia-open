...
const quizQuestions = [
  {
    ...
    answers: ["France", "Italie", "Canada"],
    correct: 0,
    explanation: "Bravo, c'est la France !",
  },
  {
    question: "Quel est le fleuve le plus long du monde ?",
    answers: ["Nil", "Amazon", "Yangtsé"],
    correct: 1,
    explanation: "L'Amazone est le plus long !",
  },
  {
    question: "En quelle année a débuté la Première Guerre mondiale ?",
    answers: ["1914", "1939", "1815"],
    correct: 0,
    explanation: "La guerre a commencé en 1914.",
  },
  {
  ...
  },
  {
    ...
    answers: ["Leonardo da Vinci", "Picasso", "Monet"],
    correct: 0,
    explanation: "C'est Léonard de Vinci !",
  },
];
...
shopsContainer.innerHTML = shops
  .map(
    (s) =>
      `<a href="${s.url}" class="shop-link" target="_blank" style="margin: 6px; display: inline-block;">${s.flag} ${s.name}</a>`,
  )
  .join(" ");
...
document.getElementById("positiviteContainer").innerHTML = motivMessages
  .map((msg) => `<div>${msg}</div>`)
  .join("");
