...
cd "$HOME/reussitess-global-nexus"

echo "▶ Correction next.config.js..."
cat > next.config.js <<'EONC'
...
echo "▶ Correction pages/legal.js..."
mkdir -p pages
cat > pages/legal.js <<'EOLG'
...
git add next.config.js pages/legal.js
git commit -m "Script fix: header Amazon + mentions légales"
