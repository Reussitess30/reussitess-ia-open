...
export default function MonitoringIA() {
  ...
  if (!stats) return <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#10b981' }}>INITIALISATION DU SYSTÈME RÉEL...</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0a0a0a 0%, #1a1a2e 50%, #0a0a0a 100%)', padding: '2rem', color: 'white', fontFamily: 'monospace' }}>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {/* REAL SECURITY ENGINE (REUSSSHIELD & BOT DESTROYER) */}
        <RealSecurityEngine />

        {/* GLOBAL HUB & FORUM TIPS */}
        <GlobalSecurityHub />
      </div>
    </div>
  )
...
function RealSecurityEngine() {
  ...
  const scanBlockchain = async () => {
    if (!(window as any).ethereum) return alert('MetaMask non détecté')
    setLoading(true)
    try {
      ...
      // Liste des routeurs et spenders courants pour scanner les permissions réelles
      const knownSpenders = [
      ...
      setDetections(found)
      if (found.length === 0) alert("SCAN RÉEL TERMINÉ : Aucun contrat suspect n'a accès à vos jetons.")
  ...
  const revokeAccess = async (spender: string) => {
    ...
    try {
      ...
      await tx.wait()
      alert("✅ ACCÈS DÉTRUIT DÉFINITIVEMENT !")
      scanBlockchain()
    } catch (e) { alert("Transaction annulée.") }
    setLoading(false)
  }
    ...
    <div style={{ marginTop: '4rem', padding: '3rem', background: '#050505', border: '2px solid #3b82f6', borderRadius: '30px' }}>
      ...
      <div style={{ padding: '2.5rem', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid #10b981', borderRadius: '20px' }}>
        ...
        <div style={{ display: 'grid', gap: '1.5rem', color: '#cbd5e1', fontSize: '0.95rem', lineHeight: '1.6' }}>
          ...
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>2. Vérification des Liquidity Pools :</strong> Vérifiez si la liquidité est "locked" sur <strong>Unicrypt</strong>.
          </div>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>3. Analyse de Contrat :</strong> Utilisez PolygonScan pour auditer les contrats. Ne validez jamais "Unlimited Approval".
          </div>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>4. Protection RPC Privé :</strong> Configurez MetaMask avec un RPC sécurisé comme <strong>MEV-Blocker</strong>.
          </div>
        </div>
      </div>
