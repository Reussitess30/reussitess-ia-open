...
function ReussShieldSection() {
  ...
  const revoke = async (spenderAddr: string) => {
    ...
    try {
      ...
      setApprovals(prev => prev.filter(a => a.address !== spenderAddr))
  ...
  return (
    <div style={{ background: 'rgba(16, 185, 129, 0.05)', border: '2px solid #10b981', borderRadius: '25px', padding: '2rem', marginBottom: '2rem' }}>
      ...
      {!wallet ? (
        <button onClick={connect} style={{display:'block', margin:'0 auto', padding:'1rem 2rem', background:'#10b981', border:'none', borderRadius:'10px', color:'white', fontWeight:'bold', cursor:'pointer'}}>🦊 FOX CONNECTER & SCANNER</button>
      ) : (
        ...
        )) : <p style={{textAlign:'center', color:'#10b981'}}>🛡️ WALLET CONNECTÉ : {wallet.slice(0,6)}...{wallet.slice(-4)} - AUCUNE MENACE DÉTECTÉE</p>
      )}
    </div>
  )
}
...
function SentinelBotDestroyer() {
  ...
  const destroyBotConnection = async (botAddr: string) => {
    ...
    try {
      ...
      await tx.wait()
      setDetectedBots(prev => prev.filter(b => b.target !== botAddr))
  ...
  return (
    <div style={{ marginTop: '2rem', padding: '2rem', background: 'rgba(239, 68, 68, 0.05)', border: '3px solid #ef4444', borderRadius: '30px', textAlign: 'center' }}>
      ...
      <p style={{ color: '#94a3b8', marginBottom: '1.5rem' }}>Analyse des permissions suspectes sur le contrat Reussitess©.</p>
      {!detectedBots.length ? (
        <button onClick={startDestructionScan} disabled={isScanning} style={{ background: '#ef4444', color: 'white', padding: '1.2rem 2.5rem', borderRadius: '15px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>
          {isScanning ? 'SCAN RÉEL EN COURS...' : 'LANCER SCAN DES BOTS'}
        </button>
      ) : (
        detectedBots.map((bot: any) => (
          <div key={bot.id} style={{ background: '#000', border: '2px solid #ef4444', padding: '1.5rem', borderRadius: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ textAlign: 'left' }}><div style={{ color: '#ef4444', fontWeight: 'bold' }}>MENACE DÉTECTÉE</div></div>
      ...
      <div style={{ padding: '2.5rem', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid #10b981', borderRadius: '20px' }}>
        ...
        <div style={{ display: 'grid', gap: '1.5rem', color: '#cbd5e1', fontSize: '0.95rem', lineHeight: '1.6' }}>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>1. La Règle d'Or "Anti-Drain" :</strong> Ne gardez jamais 100% de vos REUSS sur un wallet "chaud". Utilisez <a href="https://www.ledger.com" target="_blank" rel="noopener noreferrer" style={{color: '#10b981', textDecoration: 'none', fontWeight: 'bold'}}>Ledger</a> ou Trezor.
          </div>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>2. Vérification des Liquidity Pools :</strong> Vérifiez si la liquidité est "locked" sur <a href="https://uncx.network" target="_blank" rel="noopener noreferrer" style={{color: '#10b981', textDecoration: 'none', fontWeight: 'bold'}}>Unicrypt (UNCX)</a>.
          </div>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>3. Analyse de Contrat :</strong> Utilisez <a href="https://polygonscan.com" target="_blank" rel="noopener noreferrer" style={{color: '#10b981', textDecoration: 'none', fontWeight: 'bold'}}>PolygonScan</a> pour auditer les permissions.
          </div>
          <div style={{ borderLeft: '4px solid #10b981', paddingLeft: '1rem' }}>
            <strong style={{color: '#fff'}}>4. Protection RPC Privé :</strong> Utilisez un RPC sécurisé comme <a href="https://mevblocker.io" target="_blank" rel="noopener noreferrer" style={{color: '#10b981', textDecoration: 'none', fontWeight: 'bold'}}>MEV-Blocker</a> sur MetaMask.
          </div>
        </div>
      </div>
