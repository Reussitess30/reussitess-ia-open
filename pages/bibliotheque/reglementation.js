...
export default function Reglementation() {
    ...
    const usefulLinks = [
        ...
        { 
            title: 'DGCCRF',
            description: 'Direction Générale de la Concurrence, Consommation et Répression des Fraudes',
            tag: '🇫🇷'
        },
        { 
    ...
    return (<>
        <Head>
            ...
            <meta name="description" content="Réglementations internationales, lois et normes à jour pour le commerce électronique" />
        </Head>

        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)', padding: '20px' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ background: 'white', padding: '40px', borderRadius: '20px', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
                    ...
                    <h1 style={{ fontSize: '3em', marginBottom: '10px', color: '#7c3aed' }}>
                        ⚖️ Réglementation Internationale
                    </h1>
                    <p style={{ fontSize: '1.2em', color: '#666', marginBottom: '10px' }}>
                        Lois, normes et réglementations à jour pour le commerce international
                    </p>
                    <p style={{ fontSize: '0.95em', color: '#999', marginBottom: '40px', fontStyle: 'italic' }}>
                        ⚠️ Information à caractère général. Consultez toujours un juriste pour votre situation spécifique.
                    </p>

                    {/* Dates clés */}
                    <div style={{ background: '#f5f3ff', padding: '25px', borderRadius: '15px', marginBottom: '40px' }}>
                        <h2 style={{ color: '#7c3aed', marginBottom: '20px', fontSize: '1.5em' }}>
                            📅 Dates Clés Réglementaires
                        </h2>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                            {keyDates.map((item, i) => (
                                <div key={i} style={{ background: 'white', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                                    <div style={{ fontSize: '1.5em', fontWeight: 'bold', color: '#8b5cf6', marginBottom: '5px' }}>
                                        {item.date}
                                    </div>
                                    <div style={{ fontSize: '0.9em', color: '#666' }}>{item.event}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <div key={i} style={{ marginBottom: '30px' }}>
                            ...
                            {regulation.sections.map((section, j) => (
                                <div key={j} style={{ 
                                    ...
                                    <h3 style={{ color: '#7c3aed', marginBottom: '15px', fontSize: '1.3em' }}>
                                        {section.subtitle}
                                    </h3>
                    ...
                    {/* Liens utiles */}
                    <h2 style={{ color: '#7c3aed', marginTop: '50px', marginBottom: '25px', fontSize: '2em' }}>
                        🔗 Liens Officiels
                    </h2>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                        {usefulLinks.map((link, i) => (
                            <div key={i} style={{ 
                                ...
                                <div style={{ fontSize: '2em', marginBottom: '10px' }}>{link.tag}</div>
                                <h3 style={{ color: '#7c3aed', marginBottom: '10px' }}>{link.title}</h3>
                                <p style={{ fontSize: '0.95em', color: '#666' }}>{link.description}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ 
                        ...
                        <h2 style={{ marginBottom: '15px', fontSize: '1.8em' }}>⚠️ Avertissement Important</h2>
                        <p style={{ fontSize: '1.1em', lineHeight: '1.7', marginBottom: '15px' }}>
                            Les informations présentées sur cette page sont fournies à titre informatif et pédagogique. 
                            Elles constituent un aperçu général de la réglementation applicable au commerce international 
                            et électronique.
                        </p>
                        <p style={{ fontSize: '1.05em', lineHeight: '1.7' }}>
                            <strong>Ces informations ne constituent pas un conseil juridique.</strong> Pour toute question 
                            spécifique à votre situation, nous vous recommandons fortement de consulter un avocat spécialisé 
                            en droit commercial international ou un conseiller juridique qualifié.
                        </p>
                        <p style={{ fontSize: '0.95em', marginTop: '20px', fontStyle: 'italic' }}>
                            💡 Les réglementations évoluent régulièrement. Cette page est mise à jour périodiquement, 
                            mais vérifiez toujours les textes officiels pour les informations les plus récentes.
                        </p>
                    </div>
